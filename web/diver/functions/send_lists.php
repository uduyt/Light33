<?php 
	...
	if($data!="none"){
		...
		for($i=0;$i<$people;$i++){
			...
			$sql="INSERT INTO orders(user_id,people,contact,contact_data,datetime,entry_id,order_name,order_dni,order_dob,promo_code) VALUES ((SELECT id
																	FROM users
																	WHERE facebook_id={$fid}
																	LIMIT 1) ,{$people},'{$contact_method}','{$contact_data}','{$mydate}',{$entry_id},'{$data_array[$i]["name"]}','{$data_array[$i]["dni"]}','{$data_array[$i]["dob"]}','{$promo_code}')";
